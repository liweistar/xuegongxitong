				...
				<div class="layui-card">
					...
					<div class="layui-card-body">
						...
						<table class="layui-hide" id="test-table-operate"
							...
							<thead>
							    <tr>
							      <th lay-data="{field:'schoolnid'">ID</th>
							      <th lay-data="{field:'schoolnewtitle'">标题</th>
							      <th lay-data="{field:'schoolnewcontent'">内容</th>
							    </tr>
							</thead>
						</table>
		...
		layui
				...
				.use(
						...
						function() {
							...
							//监听表格复选框选择
							table.on('checkbox(test-table-operate)', function(
									obj) {
								console.log(obj)
							});
							//监听工具条
							table.on('tool(test-table-operate)', function(obj) {
								var data = obj.data;
								if (obj.event === 'detail') {
									layer.msg('ID：' + data.schoolnid + ' 的查看操作');
								} else if (obj.event === 'del') {
									layer.confirm('真的删除行么', function(index) {
										obj.del();
										$.getJSON("del", {
											"schoolnid" : data.schoolnid
											}, function(data) {
										})
										layer.close(index);
									});
											...
											.open({
												type : 2,
												title : '修改公告',
												content : 'listform1.jsp?schoolnid='+data.schoolnid+'',
												maxmin : true,
												area : [ '550px', '550px' ],
												btn : [ '确定', '取消' ],
							...
							table.render({
								...
								cols : [ [ {
									...
									field : 'schoolnewtitle',
									width : 220,
									title : '标题'
								}, {
									field : 'schoolnewcontent',
									width : 390,
									title : '内容',
								}, {
									width : 180,
